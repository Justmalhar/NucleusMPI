...
#include "synch.h"
#include "sysdep.h"
List<Thread*>* Thread::threadTList = new List<Thread*>();

// this is put at the top of the execution stack, for detecting stack overflows
const int STACK_FENCEPOST = 0xdedbeef;

int Thread::msgCount = 0;
...
Thread* Thread::getTByID(int threadID){ 
    ListIterator<Thread*> *listIter = new ListIterator<Thread*>(threadTList);
	    while(!listIter->IsDone())
        { 
        Thread *t = listIter->Item();
		if(t->getThreadId() == threadID){return t;}
        listIter->Next();
	}return NULL;
}

Thread* Thread::getTByName(string threadName){
	ListIterator<Thread*> *listIter = new ListIterator<Thread*>(threadTList);
	while(!listIter->IsDone()){
        Thread *t = listIter->Item();
...
bool
Thread::IsThreadAvailable(int threadID){
    ListIterator<Thread*> *listIter  = new ListIterator<Thread*>(threadTList);
    while(!listIter->IsDone()){
        Thread *t = listIter->Item();
