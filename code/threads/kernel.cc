...
Bitmap* Kernel::mBuffers = new Bitmap(100);
...
void
Kernel::clearMB(int bufferId){
		mBuffers->Clear(bufferId);
		if(msg_buff_Q.find(bufferId) != msg_buff_Q.end()) {
			std::cout << "Clearing the Message buffer: " << bufferId << std::endl;
			if(!msg_buff_Q.empty())
				msg_buff_Q.erase(bufferId);
		}
	}
