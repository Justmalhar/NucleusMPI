...
main(int argc, char **argv)
{
    int i;
    char *debugArg = "";
    //Changed this to store list of user programs in the List
    List<char*>  *userProgName = new List<char*>();        // default is not to execute a user prog
    ...
    //Code to run multiple user programs together using the userProgName list 
    //and iterator to access values and spawn new threads
    //Create an iterator to iterate through the userProgName
    
    ListIterator<char*> *userProgNameIterator = new ListIterator<char*> (userProgName);
    //Create list of threads
    List <Thread*> *userProgThreads = new List<Thread*>();
    //Append to the list each new user program to the list of threads while the list is not empty
    while (!userProgNameIterator->IsDone())
    {
        //Append to the list of user Programs
        userProgThreads->Append(new Thread(userProgNameIterator->Item()));
        //Move to the next value
	    userProgNameIterator->Next();    
    }
    //Iterator to iterate through the list of all the userProgram threads
    ListIterator<Thread*> *threadIterator = new ListIterator<Thread*> (userProgThreads);
    //Fork a new thread for each item in the list of threadIterator
    while (!threadIterator->IsDone()) 
    {
        string threadName = "";
	        threadName =  threadIterator->Item()->getName() ;
        char * progName = new char[threadName.size() + 1];
          std::copy(threadName.begin(), threadName.end(), progName);
             progName[threadName.size()] = '\0';
        //Fork RunUserProg threads
         threadIterator->Item()->Fork((VoidFunctionPtr)RunUserProg, (void *)progName);
                threadIterator->Next();    
    }
